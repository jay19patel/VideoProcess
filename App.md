...
import os
import json
from video_processing import process_video, credentials_to_dict
...
def process_video(input_video_path, output_video_path):
    ...
    main_clip = CompositeVideoClip([main_clip, logo])
    final_clip = add_intro_outro(main_clip, 'static/intro.mp4', 'static/outro.mp4', 'static/introoutro_audio.mp3')
    final_clip.write_videofile(output_video_path, codec='libx264')
...
<body class="bg-gray-100">
    <div class="container mx-auto p-8">
        ...
        <form id="upload-form" action="/upload" method="post" enctype="multipart/form-data">
            <input type="file" name="file" class="mb-4">
            <button type="submit" class="bg-blue-500 text-white px-4 py-2">Upload and Process</button>
        </form>
        <form id="youtube-upload-form" action="/upload_to_youtube" method="post" class="mt-6">
            <input type="hidden" name="video_path" value="static/processed/processed_video.mp4">
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="tags" id="tags" class="mt-1 block w-full">
            </div>
            <div class="mb-4">
                <label for="thumbnail" class="block text-sm font-medium text-gray-700">Thumbnail</label>
                <input type="file" name="thumbnail" id="thumbnail" class="mt-1 block w-full">
            </div>
            <button type="submit" class="bg-green-500 text-white px-4 py-2">Upload to YouTube</button>
        </form>
        <div id="progress-bar" class="hidden mt-6 w-full bg-gray-200 rounded-full">
            <div id="progress" class="bg-blue-500 text-xs leading-none py-1 text-center text-white rounded-full" style="width: 0%">0%</div>
        </div>
        <div id="completion-message" class="hidden mt-6 text-green-500">Upload Complete!</div>
    </div>
    <script>
        ...
        uploadForm.addEventListener('submit', function(e) {
            ...
            .then(data => {
                // Handle success, update the video path in the YouTube upload form
                document.querySelector('input[name="video_path"]').value = data.video_path;
            });
        });

        youtubeUploadForm.addEventListener('submit', function(e) {
            e.preventDefault();
